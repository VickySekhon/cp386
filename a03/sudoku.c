...
int result[NUM_THREADS] = {
    0}; // result array to store validity of each thread's check

int main()
{
  readSudokuPuzzleFromFile("sample_in_sudoku.txt");
  ...
  for (int i = 0; i < NUM_THREADS; i++)
  {
    ...
    {
      isValid = 0;
      break; // stop at point where 0 (invalid) is encountered
    }
  }
  ...
  for (int i = 0; i < SIZE; i++)
  {
    ...
    for (int j = 0; j < SIZE; j++)
    {
      ...
      {                // check if number is valid
        result[0] = 0; // set result to 0 if invalid
        pthread_exit(NULL);
      }
      else
      {
...
void *validateSubgrid(void *param)
{
  ...
  {
    for (int j = p->column; j < p->column + 3;
         j++) // stop condition = 3rd column
    {
